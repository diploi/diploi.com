...
const isOverlapping = (sparkleA: { left: number; top: number; size: number }, sparkleB: { left: number; top: number; size: number }) => {
...
export const createSparkles = (sparkleCount: number, minSize: number, maxSize: number) => {
  ...
  for (let i = 0; i < sparkleCount; i++) {
    ...
    do {
      sparkle = generateSparkle(minSize, maxSize, i);
      attempts++;
